...
Uses FastText word embeddings (padded sequences) as input features.
"""
...
fasttext_model = KeyedVectors.load('../Assignment_2/fasttext-wiki-news-subwords-300.model')
emb_dim = fasttext_model.vector_size
print(f"FastText model loaded. Embedding dimension: {emb_dim}")

# ========== Compute Padded Sequence Embeddings ==========
print("\n========== Computing Padded Sequence Embeddings ==========")
max_seq_len = 32

def sentence_to_padded_embeddings(sentence, model, emb_dim, max_len):
    """Convert sentence to padded sequence of FastText word vectors. OOV words get zero vectors."""
...
X_seq = np.array([sentence_to_padded_embeddings(sent, fasttext_model, emb_dim, max_seq_len)
                   for sent in tqdm(data['sentence'], desc="Embedding sentences")])
y = data['label'].values
...
class LSTMClassifier(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.cat([h_n[-2], h_n[-1]], dim=1)  # (batch, hidden_dim*2)
        return self.fc(out)
